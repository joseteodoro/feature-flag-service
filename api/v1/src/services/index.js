...
// const db = require('../../../../db')
const featureRepository = require('../models/repositories/feature')
...
const listFeatures = ({ author, enabled = true, page = 0, pageSize = 10 }) => {
  ...
  return featureRepository.list({where, limit, offset})
}

const findFeature = (mnemonic, { author, enabled = true }) =>
  featureRepository.findOne(mnemonic, { enabled, ...safeAuthor(author) })

const addFeature = feature =>
  featureRepository.findOne(feature.mnemonic)
    .then(create)

const updateFeature = (mnemonic, args) => featureRepository.findOne(mnemonic, args)
